function export_obj(XYZ,N,rho,mask,filename)
%EXPORT_OBJ exports a gridded oriented point cloud to a .obj file
%	EXPORT_OBJ(XYZ,N) writes the point cloud XYZ with normals N to file
%	'mesh.obj'. XYZ must be M x N x 3, and N must be M x N x 3
%
%	EXPORT_OBJ(XYZ,N,RHO) uses the albedo RHO as texture map. RHO must 
%	be M x N x 1 (graylevel) or M x N x 3 (RGB), and of class uint8
%
%	EXPORT_OBJ(XYZ,N,RHO,MASK) writes only the points inside MASK. 
%	MASK must be an M x N binary matrix. 
%
%	EXPORT_OBJ(XYZ,N,RHO,MASK,FILENAME) writes the result to file
%	FILENAME.OBJ
%
%	Credits: 
%	- Yvain Queau, Technical University Munich, May 2017
%	- Calls the 'write_wobj' function by D.Kroon, University of Twente

	if(nargin<2)
		disp('ERROR: at least two arguments must be provided');
	end

	if (~exist('filename','var')|isempty(filename)) filename='mesh'; end;
	if (~exist('rho','var')|isempty(rho)) rho=0.8*ones(size(XYZ)); end;
